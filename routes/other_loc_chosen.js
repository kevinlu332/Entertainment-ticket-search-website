...
/* GET home page. */
router.get('/other_loc_chosen', function(req, res, next) {
    var q1 = url.parse(req.url, true);
    var params1=q1.query;

    // from client side
    // url1 = "LocAddress=" + LocAddress + '&' + "Keyword=" + $scope.form.keyword + '&' + "Category=" + $scope.userCategory + '&' + "Distance=" + acturalDistance;
    var SegmentId;
    ...
    let aaddress;
    aaddress = params1.LocAddress;
    aaddress = aaddress.split(' ').join('+');

    var ghash;
    let url0 ="https://maps.googleapis.com/maps/api/geocode/json?address="+ aaddress + "&key=AIzaSyA8XjdOsEdYtgaETUMOuR8A20puTbTGVp0";
    https.get(url0,function(req3, res3){
        let res_text3 = "";
        ...
        req3.on('end',function() {
            ...
            let paramsOfOtherLoc = JSON.parse(res_text3);
            if (paramsOfOtherLoc.status === "ZERO_RESULTS") {
                res.send("location error");
            }
            else {
                let addsf = paramsOfOtherLoc.results[0].geometry.location.lat;

                ghash = ngeohash.encode(paramsOfOtherLoc.results[0].geometry.location.lat, paramsOfOtherLoc.results[0].geometry.location.lng);


                let url1 = 'https://app.ticketmaster.com/discovery/v2/events.json?apikey=yGT4UZmzzJAMkAAHwapvuMWivbUe4l79' + '&';
                url1 += "keyword=" + kkeyword + '&' + "segmentId=" + SegmentId + '&' + "radius=" + params1.Distance + '&' + "unit=" + params1.Unit + "&" + "geoPoint=" + ghash;
                console.log(url1);
...
// the rest is the same as the part of curr_loc_chosen
                        var obj1 = JSON.parse(res_text1);
                        if (obj1.page.totalElements === 0) {
                            res.send("No results");
                        }
                        else {
                            ...
                            key1[13]='D_city';
                            key1[14]='id';
                            key1[15]='attract_names';
                            key1[16]='vID';
                            key1[17]='vLat'; key1[18]='vLon';
                            key1[19]='segment';
                            var events = {};
                            events[0] = [];
                            for (let j = 0; j < obj1._embedded.events.length; j++) {
                                ...
                                if (obj1._embedded.events[j].seatmap.staticUrl !== undefined) {
                                ...
                                if (obj1._embedded.events[0]._embedded.venues!==undefined) {
                                    mysubobj1[key1[16]]=obj1._embedded.events[0]._embedded.venues[0].id;
                                }
            ...
            }  //this line is extra and needed in this file and different from curr_loc_chosen
        });   //this line is extra and needed in this file and different from curr_loc_chosen
    });   //this line is extra and needed in this file and different from curr_loc_chosen
});
